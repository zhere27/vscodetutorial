    ...
    public class ApplicationDbContextSeed
    {
            ...
            ILoggerFactory loggerFactory, int? retry = 0)
        {
            ...
            try
            {
                ...
                if (!ApplicationDbContext.CatalogBrands.Any())
                {
                    ApplicationDbContext.CatalogBrands.AddRange(
                        GetPreconfiguredCatalogBrands());

                    await ApplicationDbContext.SaveChangesAsync();
                }

                if (!ApplicationDbContext.CatalogTypes.Any())
                {
                    ApplicationDbContext.CatalogTypes.AddRange(
                        GetPreconfiguredCatalogTypes());

                    await ApplicationDbContext.SaveChangesAsync();
                }

                if (!ApplicationDbContext.CatalogItems.Any())
                {
                    ApplicationDbContext.CatalogItems.AddRange(
